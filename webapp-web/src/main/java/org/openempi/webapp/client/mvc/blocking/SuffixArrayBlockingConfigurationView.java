...
public class SuffixArrayBlockingConfigurationView extends View
{
	...
	private ListStore<ModelPropertyWeb> similarityMetricStore = new ListStore<ModelPropertyWeb>();
	private Grid<BaseFieldWeb> addRoundGrid;
	private SpinnerField minimumSuffixSize;
	private SpinnerField maximumBlockSize;
	
	private ComboBox<ModelPropertyWeb> similarityMetricCombo = new ComboBox<ModelPropertyWeb>();
	private NumberField similarityThresholdEdit = new NumberField();
	
	private LayoutContainer container;
	
	@SuppressWarnings("unchecked")
	public SuffixArrayBlockingConfigurationView(Controller controller) {
		super(controller);
		
		//  Similarity Metric Names are same as Comparator Function Names
		List<ModelPropertyWeb> similarityMetricNames = (List<ModelPropertyWeb>) Registry.get(Constants.COMPARATOR_FUNCTION_NAMES); 
		...
		try {
			listStore.add(names);
			similarityMetricStore.add(similarityMetricNames);
		} catch (Exception e) {
	...
	protected void handleEvent(AppEvent event) {
		if (event.getType() == AppEvents.SuffixArrayBlockingConfigurationView) {
			initUI();
		} else if (event.getType() == AppEvents.SuffixArrayBlockingConfigurationReceived) {
			store.removeAll();
			SuffixArrayBlockingConfigurationWeb suffixArray = (SuffixArrayBlockingConfigurationWeb) event.getData();
			
			// display values
			minimumSuffixSize.setValue(suffixArray.getMinimumSuffixLength());	
			maximumBlockSize.setValue(suffixArray.getMaximumBlockSize());	
			List<BaseFieldWeb> fields = (List<BaseFieldWeb>) suffixArray.getBlockingRounds();						
			for (BaseFieldWeb baseField : fields) {				
				baseField.setFieldDescription(Utility.convertToDescription(baseField.getFieldName()));
				store.add(baseField);
			}
			
			// Info.display("Information", "Similarity Metric: "+suffixArray.getSimilarityMetric());
			if(suffixArray.getSimilarityMetric() != null ) {
				similarityMetricCombo.setValue(new ModelPropertyWeb(suffixArray.getSimilarityMetric(), Utility.convertToDescription(suffixArray.getSimilarityMetric())));	
				similarityThresholdEdit.setValue( Float.parseFloat( nfc.format( suffixArray.getSimilarityThreshold() )));	

			} else {
				similarityMetricCombo.clear();
				
				similarityThresholdEdit.clear();
				similarityThresholdEdit.disable();
			}	

		} else if (event.getType() == AppEvents.SuffixArrayBlockingConfigurationSaveComplete) {			
			// String message = event.getData();
			// Info.display("Information", "Person was successfully added with message " + message);					
	        MessageBox.alert("Information", "Suffix Array Blocking Configuration was successfully saved", null);  	 
	...
	private void initUI() {
		...
		controller.handleEvent(new AppEvent(AppEvents.SuffixArrayBlockingConfigurationRequest));
...
//		ColumnConfig fieldName = new ColumnConfig("fieldName", "Field Name", 100);
		...
		cp.setHeading("Suffix Array Blocking Configuration");
		cp.setFrame(true);
	    ...
	    LabelField minimumSuffixSizeLabel = new LabelField("Minimum Suffix Size:");
	    minimumSuffixSize = new SpinnerField();
	    minimumSuffixSize.setAllowDecimals(false);
	    minimumSuffixSize.setAllowNegative(false);
	    minimumSuffixSize.setMinValue(2);
	    minimumSuffixSize.setMaxValue(20);
	    minimumSuffixSize.setWidth(50);

		// add some space for error icon
		Status suffixStatus = new Status();
		suffixStatus.setText("");
		suffixStatus.setWidth(20);

	    LabelField maximumBlockSizeLabel = new LabelField("Maximum Block Size:");
	    maximumBlockSize = new SpinnerField();
	    maximumBlockSize.setAllowDecimals(false);
	    maximumBlockSize.setAllowNegative(false);
	    maximumBlockSize.setMinValue(2);
	    maximumBlockSize.setMaxValue(20);
	    maximumBlockSize.setWidth(50);

		// add some space for error icon
		Status blockSizeStatus = new Status();
		blockSizeStatus.setText("");
		blockSizeStatus.setWidth(20);
	    
		toolBar.add(new FillToolItem());
		toolBar.add(minimumSuffixSizeLabel);		
		toolBar.add(minimumSuffixSize);	
	    toolBar.add(suffixStatus);
		toolBar.add(maximumBlockSizeLabel);		
		toolBar.add(maximumBlockSize);	
	    toolBar.add(blockSizeStatus);
		...
		c.add(new Button("Save Settings", IconHelper.create("images/folder_go.png"), new SelectionListener<ButtonEvent>() {
	          ...
	          public void componentSelected(ButtonEvent ce) {
				  ...
				  List<ModelPropertyWeb> similarityMetricSelection = similarityMetricCombo.getSelection();
					
	        	  if ( minimumSuffixSize.isValid() && maximumBlockSize.isValid() && similarityThresholdEdit.isValid() ) {	
	        		  
	        		  SuffixArrayBlockingConfigurationWeb configuration = new SuffixArrayBlockingConfigurationWeb();
		  			
		  			  configuration.setMinimumSuffixLength( minimumSuffixSize.getValue().intValue() );
		  			  configuration.setMaximumBlockSize( maximumBlockSize.getValue().intValue() );


		  			  configuration.setBlockingRounds( grid.getStore().getModels() );		

  					  if( similarityMetricSelection.size() > 0) {
  						  ModelPropertyWeb similarityMetricField = similarityMetricSelection.get(0);
					      configuration.setSimilarityMetric(similarityMetricField.getName());							  
  					  } /* else {
		  				  configuration.setSimilarityMetric("");			  		
  					  } */
  					  
		  			  if( similarityThresholdEdit.getValue() != null ) {
		  				  configuration.setSimilarityThreshold(similarityThresholdEdit.getValue().floatValue());	
		  			  } /* else {
		  				  configuration.setSimilarityThreshold(0f);
		  			  } */
		  			  
		        	  controller.handleEvent(new AppEvent(AppEvents.SuffixArrayBlockingConfigurationSave, configuration));		        	  
		        	  
	        	  } else {
	        		  Info.display("Information","The some field is not valid.");
	        	  }
	          }
	    }), layoutData); 
		...
		similarityMetricCombo.setEmptyText("Select function...");
		similarityMetricCombo.setForceSelection(true);
		// comparatorFuncNameCombo.setDisplayField("name");
		similarityMetricCombo.setDisplayField("description");
		similarityMetricCombo.setStore(similarityMetricStore);
		similarityMetricCombo.setTypeAhead(true);
		similarityMetricCombo.setTriggerAction(TriggerAction.ALL);
		similarityMetricCombo.setFieldLabel("Similarity Metric");
		
		// Edit change
		similarityMetricCombo.addListener(Events.OnChange, new Listener<BaseEvent>() {
	        public void handleEvent(BaseEvent be) {				
				  List<ModelPropertyWeb> similarityMetricSelection = similarityMetricCombo.getSelection();					
				  if( similarityMetricSelection.size() > 0) {
					  similarityThresholdEdit.enable();						  
				  } else {
					  similarityThresholdEdit.clear();
					  similarityThresholdEdit.disable();
				  }
		...
		// Select change
		similarityMetricCombo.addListener(Events.SelectionChange, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
				  List<ModelPropertyWeb> similarityMetricSelection = similarityMetricCombo.getSelection();					
				  if( similarityMetricSelection.size() > 0) {
					  similarityThresholdEdit.enable();						  
		...
		cp.add(similarityMetricCombo);
		
		similarityThresholdEdit.setFieldLabel("Similarity Threshold");
 		similarityThresholdEdit.setAllowBlank(false);
		similarityThresholdEdit.setMinValue(0);
		similarityThresholdEdit.setMaxValue(1);
		cp.add(similarityThresholdEdit);
