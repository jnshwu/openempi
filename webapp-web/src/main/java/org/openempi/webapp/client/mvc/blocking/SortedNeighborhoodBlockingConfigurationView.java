...
public class SortedNeighborhoodBlockingConfigurationView extends View
{
	...
	private SpinnerField slidingWindowSize;
	
	private LayoutContainer container;
	
	@SuppressWarnings("unchecked")
	public SortedNeighborhoodBlockingConfigurationView(Controller controller) {
		super(controller);
	...
	protected void handleEvent(AppEvent event) {
		if (event.getType() == AppEvents.SortedNeighborhoodBlockingConfigurationView) {
			initUI();
		} else if (event.getType() == AppEvents.SortedNeighborhoodBlockingConfigurationReceived) {
			store.removeAll();
			SortedNeighborhoodConfigurationWeb sortedNeighbor = (SortedNeighborhoodConfigurationWeb) event.getData();
			
			// display values
			slidingWindowSize.setValue(sortedNeighbor.getWindowSize());			
			List<BaseFieldWeb> fields = (List<BaseFieldWeb>) sortedNeighbor.getBlockingRounds();						
			for (BaseFieldWeb baseField : fields) {				
				baseField.setFieldDescription(Utility.convertToDescription(baseField.getFieldName()));
				store.add(baseField);
			}
		} else if (event.getType() == AppEvents.SortedNeighborhoodBlockingConfigurationSaveComplete) {			
			// String message = event.getData();
			// Info.display("Information", "Person was successfully added with message " + message);					
	        MessageBox.alert("Information", "Sorted Neighborhood Blocking Configuration was successfully saved", null);  	 
	...
	private void initUI() {
		...
		controller.handleEvent(new AppEvent(AppEvents.SortedNeighborhoodBlockingConfigurationRequest));
...
//		ColumnConfig fieldName = new ColumnConfig("fieldName", "Field Name", 100);
		...
		cp.setHeading("Sorted Neighborhood Blocking Configuration");
		cp.setFrame(true);
	    ...
	    LabelField slidingWindowSizeLabel = new LabelField("Sliding Window Size:");
		slidingWindowSize = new SpinnerField();
		slidingWindowSize.setAllowDecimals(false);
		slidingWindowSize.setAllowNegative(false);
		// slidingWindowSize.setFieldLabel("Sliding Window Size");
		slidingWindowSize.setMinValue(2);
		slidingWindowSize.setMaxValue(100);
		slidingWindowSize.setWidth(50);
		...
		toolBar.add(slidingWindowSizeLabel);		
		toolBar.add(slidingWindowSize);	
	    toolBar.add(status);
		...
		c.add(new Button("Save Settings", IconHelper.create("images/folder_go.png"), new SelectionListener<ButtonEvent>() {
	          ...
	          public void componentSelected(ButtonEvent ce) {
	        	  ...
	        	  if ( slidingWindowSize.isValid()) {
		  			  SortedNeighborhoodConfigurationWeb configuration = new SortedNeighborhoodConfigurationWeb();
		  			
		  			  configuration.setWindowSize( slidingWindowSize.getValue().intValue() );
		  			  configuration.setBlockingRounds( grid.getStore().getModels() );					        	  
		        	  controller.handleEvent(new AppEvent(AppEvents.SortedNeighborhoodBlockingConfigurationSave, configuration));
	        	  } else {
	        		  Info.display("Information","The number of Window Size is not valid.");
	        	  }
	          }
	    }), layoutData); 
