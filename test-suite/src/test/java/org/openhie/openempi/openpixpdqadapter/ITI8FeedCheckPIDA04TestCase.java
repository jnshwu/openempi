...
public class ITI8FeedCheckPIDA04TestCase extends AbstractPixTest
{
/*
		Pre-condition: 	
...
*/
	   public void testFeedCheckPIDA04() {
/*
 *		Messages:
 * 
...
*/
			try {
				...
				String message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160420||ADT^A04^ADT_A01|NIST-101101160420696|P|2.3.1||||||||\r" + 
					      "EVN||20101020||||\r" +
				...
				assertEquals("NIST-101101160420696", msa.getMessageControlID().getValue());
				
				
				
				//Step 2: Register patient JOHNSTON^ROBERT with RJ-438^^^&&ISO 
				message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160431||ADT^A04^ADT_A01|NIST-101101160431597|P|2.3.1||||||||\r" + 
					      "EVN||20101020||||\r" +
				...
				assertEquals("AE", msa.getAcknowledgementCode().getValue());
				assertEquals("NIST-101101160431597", msa.getMessageControlID().getValue());	
				
			} catch (Exception e) {
				e.printStackTrace();
				fail("Fail to test FeedCheckPIDA04.");
			}					
