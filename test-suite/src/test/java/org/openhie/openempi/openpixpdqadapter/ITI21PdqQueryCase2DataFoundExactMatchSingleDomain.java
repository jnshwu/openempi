...
public class ITI21PdqQueryCase2DataFoundExactMatchSingleDomain  extends AbstractPdqTest
{
/*
		Pre-condition: 	
		...
		Step 1: The NIST PDQ Consumer sends a query message (QBP^Q22) to ask for the demographics of all patients with a patient name GERALD BXRACK using NIST2010&2.16.840.1.113883.3.72.5.9.1&ISO in QPD-8. Your PDQ Supplier shall answer correctly to the query with the demographics of patient GERALD BXRACK in domain NIST2010&2.16.840.1.113883.3.72.5.9.1&ISO.

		
*/
	   public void testPdqQueryCase2DataFoundExactMatchSingleDomain() {
/*
 *		Messages:
 * 
		MSH|^~\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101103152224||QBP^Q22^QBP_Q21|NIST-101103152224809|D|2.5
		QPD|IHE PDQ Query|QRY1184848949494|@PID.5.1.1^BXRACK~@PID.5.2^GERALD|||||^^^NIST2010
		RCP|I
...
*/
			try {
				...
				String message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101103152224||QBP^Q22^QBP_Q21|NIST-101103152224809|D|2.5||||||||\r" + 
					      "QPD|IHE PDQ Query|QRY1184848949494|@PID.5.1.1^BXRACK~@PID.5.2^GERALD|||||^^^NIST2010\r" +
					      "RCP|I||||||";
				...
				assertEquals("NIST-101103152224809", msa25.getMessageControlID().getValue());
				...
				e.printStackTrace();
				fail("Fail to test PdqQueryCase2DataFoundExactMatchSingleDomain.");
			}					
