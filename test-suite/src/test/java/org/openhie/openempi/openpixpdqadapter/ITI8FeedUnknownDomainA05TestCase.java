...
public class ITI8FeedUnknownDomainA05TestCase extends AbstractPixTest
{
/*
		Pre-condition: 	
		...
		Description:
		The purpose of this test is to check that a PIX Manager does not accept a feed message (ADT^A05^ADT_A01) from an unknown domain. An unknown domain is a valid domain but it is not recognized by the PIX Manager.
...
*/
	   public void testFeedUnknownDomainA05() {
/*
 *		Messages:
 * 
		MSH|^~\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160607||ADT^A05^ADT_A01|NIST-101101160607820|P|2.3.1
		EVN||20101020
		...
		MSH|^~\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160618||ADT^A05^ADT_A01|NIST-101101160618425|P|2.3.1
		EVN||20101020
		...
		MSH|^~\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160629||ADT^A05^ADT_A01|NIST-101101160629123|P|2.3.1
		EVN||20101020
...
*/
			try {
				...
				String message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160607||ADT^A05^ADT_A01|NIST-101101160607820|P|2.3.1||||||||\r" + 
					      "EVN||20101020||||\r" +
				...
				assertEquals("NIST-101101160607820", msa.getMessageControlID().getValue());
				
				
				
				//Step 2: PIX feed to create one patient from domain UNKNOWNDOMAIN
				message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160618||ADT^A05^ADT_A01|NIST-101101160618425|P|2.3.1||||||||\r" + 
					      "EVN||20101020||||\r" +
				...
				assertEquals("AE", msa.getAcknowledgementCode().getValue());
				assertEquals("NIST-101101160618425", msa.getMessageControlID().getValue());
				
				
				//Step 3: PIX feed to create one patient from domain 2.16.840.1.113883.3.72.5.9.99&ISO
				message = "MSH|^~\\&|NIST_SENDER^^|NIST^^|NIST_RECEIVER^^|NIST^^|20101101160629||ADT^A05^ADT_A01|NIST-101101160629123|P|2.3.1||||||||\r" + 
					      "EVN||20101020||||\r" +
				...
				msa = (MSA)response.get("MSA");
				assertEquals("AE", msa.getAcknowledgementCode().getValue());
				assertEquals("NIST-101101160629123", msa.getMessageControlID().getValue());				
				
			} catch (Exception e) {
				e.printStackTrace();
				fail("Fail to test FeedUnknownDomainA05.");
			}					
